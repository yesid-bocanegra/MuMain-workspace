...
- [ ] **AC-1:** `MuMain/cmake/toolchains/macos-arm64.cmake` exists with Clang compiler configuration, `CMAKE_CXX_STANDARD 20`, extensions OFF, and correct system framework paths (CoreFoundation, IOKit, etc.)
- [ ] **AC-2:** `MuMain/CMakePresets.json` includes `macos-arm64` configure preset and associated build presets (`macos-arm64-debug`, `macos-arm64-release`)
- [ ] **AC-3:** `cmake --preset macos-arm64` succeeds on macOS arm64 (configure step completes — full compile not expected until SDL3 migration removes Win32 API dependencies)
- [ ] **AC-4:** Existing Windows MSVC presets (`windows-x64`, `windows-x64-mueditor`, `windows-x86`, `windows-x86-mueditor`) are unchanged and all their build presets still function on Windows
...
- [ ] **AC-STD-11:** Flow Code traceability — commit message includes `VS0-PLAT-CMAKE-MACOS` reference
...
- [ ] **AC-STD-4:** CI (MinGW cross-compile) quality gate remains green — the macOS presets must not break or conflict with the existing MinGW CI workflow
...
- [ ] **AC-VAL-1:** macOS configure log showing successful `cmake --preset macos-arm64` run (paste terminal output as comment or attach as artifact)
- [ ] **AC-VAL-2:** Windows build confirmed not regressed — run `cmake --preset windows-x64` + `cmake --build --preset windows-x64-debug` on Windows and confirm green

---

## Tasks / Subtasks

- [ ] **Task 1: Create macOS arm64 toolchain file** (AC: AC-1)
  - [ ] 1.1 Create `MuMain/cmake/toolchains/macos-arm64.cmake`
  - [ ] 1.2 Set `CMAKE_SYSTEM_NAME Darwin`, `CMAKE_SYSTEM_PROCESSOR arm64`
  - [ ] 1.3 Configure Clang as C and C++ compiler (`clang` / `clang++`)
  - [ ] 1.4 Set `CMAKE_CXX_STANDARD 20`, `CMAKE_CXX_EXTENSIONS OFF`
  - [ ] 1.5 Add macOS system framework paths (SDK path via `xcrun --show-sdk-path`)
  - [ ] 1.6 Set deployment target: `CMAKE_OSX_DEPLOYMENT_TARGET "12.0"` (or whichever minimum is appropriate — see Dev Notes)
  - [ ] 1.7 Add `-arch arm64` to `CMAKE_OSX_ARCHITECTURES`

- [ ] **Task 2: Update CMakePresets.json with macOS presets** (AC: AC-2)
  - [ ] 2.1 Add `macos-base` hidden configure preset (mirrors `windows-base` pattern)
  - [ ] 2.2 Add `macos-arm64` configure preset inheriting `macos-base`, using `cmake/toolchains/macos-arm64.cmake`
  - [ ] 2.3 Set `CMAKE_EXPORT_COMPILE_COMMANDS ON` in macOS presets (needed for clang-tidy/clangd)
  - [ ] 2.4 Add `macos-arm64-debug` and `macos-arm64-release` build presets
  - [ ] 2.5 Ensure the new presets do NOT add a `"condition"` that breaks validation on other host OSes (use `hostSystemName` filter if needed to avoid confusion, but verify CI behavior)

- [ ] **Task 3: Validate configure step on macOS** (AC: AC-3)
  - [ ] 3.1 Run `cmake --preset macos-arm64` on macOS arm64 hardware
...
- [ ] **Task 4: Regression check — Windows presets** (AC: AC-4)
  - [ ] 4.1 On Windows (or verify in CI): `cmake --preset windows-x64` still configures successfully
  - [ ] 4.2 `cmake --build --preset windows-x64-debug` still builds successfully
  - [ ] 4.3 Confirm MinGW CI workflow in `MuMain/.github/workflows/ci.yml` still passes (no changes to CI file needed)
