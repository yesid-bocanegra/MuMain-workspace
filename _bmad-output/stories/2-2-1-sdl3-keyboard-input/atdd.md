...
- [x] `1.1` Add `VK_LCONTROL 0xA2` to PlatformKeys.h (non-Windows block)
- [x] `1.2` Add `VK_SNAPSHOT 0x2C` to PlatformKeys.h (non-Windows block)
- [x] `1.3` Add `VK_CAPITAL 0x14`, `VK_NUMLOCK 0x90`, `VK_SCROLL 0x91`, `VK_PAUSE 0x13`
- [x] `1.4` Add `VK_OEM_1 0xBA`, `VK_OEM_PLUS 0xBB`, `VK_OEM_COMMA 0xBC`, `VK_OEM_MINUS 0xBD`, `VK_OEM_PERIOD 0xBE`
- [x] `1.5` Add `VK_MULTIPLY 0x6A`, `VK_ADD 0x6B`, `VK_SEPARATOR 0x6C`, `VK_SUBTRACT 0x6D`, `VK_DECIMAL 0x6E`, `VK_DIVIDE 0x6F`
- [x] `1.6` Add `VK_LSHIFT 0xA0`, `VK_RSHIFT 0xA1`, `VK_RCONTROL 0xA3`, `VK_LMENU 0xA4`, `VK_RMENU 0xA5`

### Task 2: Add GetAsyncKeyState shim to PlatformCompat.h (AC-1, AC-4)

- [x] `2.1` Add `HIBYTE` macro (non-Windows, `#ifndef HIBYTE` guard): `#define HIBYTE(w) (static_cast<uint8_t>((static_cast<uint16_t>(w) >> 8) & 0xFF))`
- [x] `2.2` Check PlatformTypes.h for `BYTE` typedef — add `using BYTE = uint8_t;` if missing (non-Windows)
- [x] `2.3` Add `extern bool g_sdl3KeyboardState[512]` declaration in PlatformCompat.h (inside `#ifdef MU_ENABLE_SDL3` block)
- [x] `2.4` Add `MuVkToSdlScancode(int vk)` inline function implementing full VK->SDL_Scancode table from Dev Notes
- [x] `2.5` Add `GetAsyncKeyState(int vk)` inline shim (inside `#ifdef MU_ENABLE_SDL3` inside `#else // !_WIN32`):
...
- [x] `2.6` Verify flow code `VS1-SDL-INPUT-KEYBOARD` appears in `g_ErrorReport.Write()` call (AC-STD-11, AC-STD-14)
- [x] `2.7` Add `static_assert(SDL_SCANCODE_A == 4)` and `static_assert(SDL_SCANCODE_1 == 30)` (Task 5.4)

### Task 3: Define g_sdl3KeyboardState array (AC-1, AC-4)

- [x] `3.1` Choose implementation: Option A (SDLKeyboardState.cpp) or Option B (inline C++20 in PlatformCompat.h)
- [x] `3.2` If Option A: Create `MuMain/src/source/Platform/sdl3/SDLKeyboardState.cpp` with `bool g_sdl3KeyboardState[512] = {};` inside `#ifdef MU_ENABLE_SDL3`
- [x] `3.3` If Option A: Add `SDLKeyboardState.cpp` to MUPlatform CMake target inside `if(MU_ENABLE_SDL3)` block

### Task 4: Feed keyboard state from SDLEventLoop (AC-1, AC-4)

- [x] `4.1` Add `SDL_EVENT_KEY_DOWN` handler in `SDLEventLoop.cpp PollEvents()`: set `g_sdl3KeyboardState[event.key.scancode] = true` (with bounds check `< 512`)
- [x] `4.2` Add `SDL_EVENT_KEY_UP` handler: set `g_sdl3KeyboardState[event.key.scancode] = false`
- [x] `4.3` In `HandleFocusLoss()`: clear all entries — `std::fill(std::begin(g_sdl3KeyboardState), std::end(g_sdl3KeyboardState), false)` (prevents stuck keys on Alt-Tab)
- [x] `4.4` Add `#include <algorithm>` if not already present in SDLEventLoop.cpp

### Task 5: ASCII and extended key mappings (AC-2, AC-3)

- [x] `5.1` In `MuVkToSdlScancode()`: add range mapping for 'A'-'Z' (0x41-0x5A) -> `SDL_SCANCODE_A + (vk - 'A')`
- [x] `5.2` In `MuVkToSdlScancode()`: add range mapping for '1'-'9' (0x31-0x39) -> `SDL_SCANCODE_1 + (vk - '1')`
- [x] `5.3` In `MuVkToSdlScancode()`: add mapping for '0' (0x30) -> `SDL_SCANCODE_0`
- [x] `5.4` Add compile-time `static_assert` for SDL scancode values (verified in test_platform_input.cpp)

### Task 6: Tests (AC-STD-2, AC-STD-16)

- [x] `6.1` Verify `test_platform_input.cpp` compiles with `MU_ENABLE_SDL3=ON` (all TEST_CASEs fail — RED phase)
- [x] `6.2` Verify `test_platform_input.cpp` compiles with `MU_ENABLE_SDL3=OFF` (empty TU — no errors — CI safe)
- [x] `6.3` Verify `test_ac_std11_flow_code_2_2_1.cmake` FAILS before implementation (PlatformCompat.h has no flow code yet)
- [x] `6.4` Verify `test_ac_std3_no_raw_getasynckeystate.cmake` passes (no unexpected new GetAsyncKeyState calls)
- [x] `6.5` Verify new tests registered in `MuMain/tests/CMakeLists.txt` and `MuMain/tests/platform/CMakeLists.txt`

### Task 7: Quality Gate (AC-STD-13)

- [x] `7.1` `make -C MuMain format-check` passes (Allman braces, 4-space, 120-col, LF, UTF-8)
- [x] `7.2` `make -C MuMain lint` passes (zero cppcheck warnings in new Platform/ files)
- [x] `7.3` `./ctl check` passes on macOS
- [x] `7.4` All new SDL3 code inside `#ifdef MU_ENABLE_SDL3` — MinGW CI build unaffected

### PCC Compliance Items

- [x] No prohibited libraries used (no raw new/delete, no NULL, no wprintf in new code)
- [x] Required patterns present: `[[nodiscard]]` on fallible functions, `g_ErrorReport.Write()` with `MU_ERR_*` prefix
- [x] MU_ENABLE_SDL3 compile-time guard on all SDL3 code
- [x] Catch2 v3.7.1 via FetchContent, tests in `MuMain/tests/platform/`, BUILD_TESTING=ON opt-in
- [x] VS1-SDL-INPUT-KEYBOARD flow code in log messages AND test names AND story artifacts
- [x] No `#ifdef _WIN32` in game logic files — only in Platform/ abstraction layer and PlatformCompat.h
- [x] `#pragma once` only (no `#ifndef` guards) in all new headers

### AC-STD-3: No Unshimmed Calls

- [x] All 8 known GetAsyncKeyState call-site files compile through shim with zero source changes
- [x] CMake script test `2.2.1-AC-STD-3:no-raw-getasynckeystate` passes

### Error Catalog (AC-STD-8, AC-STD-14)

- [x] `MU_ERR_INPUT_UNMAPPED_VK` added to `docs/error-catalog.md`
- [x] Error message format: `MU_ERR_INPUT_UNMAPPED_VK [VS1-SDL-INPUT-KEYBOARD]: unmapped VK code 0x{XX}`
- [x] Logged via `g_ErrorReport.Write()` (not wprintf, not g_ConsoleDebug)
