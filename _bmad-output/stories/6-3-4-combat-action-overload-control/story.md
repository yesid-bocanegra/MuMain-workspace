...
- [x] Only superseded OpenMU basic monster attack animations (`0x18`, action `120`) coalesce per entity.
...
- Coalescing runs after frame-batch capture and must recognize only OpenMU basic monster attack animations (`0x18`, action `120`). Unknown and special actions remain ordered.
...
- Queue diagnostics emit at most once per second when `MU_NETWORK_DIAGNOSTICS=1`.
- OpenMU contract verified on 2026-07-17: basic monster attacks use animation `120`; authoritative damage and monster skill animation use separate packets.
